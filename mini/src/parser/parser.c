...
static char	**realloc_args(char **args, int count, t_gc *gc)
{
	char	**new_args;
	int		i;

	new_args = gc_malloc(sizeof(char *) * (count + 2), gc);
	if (!new_args)
		return (NULL);
	i = 0;
	if (args)
...
int	add_arg_to_cmd(t_cmd *cmd, char *arg, t_env *env, t_gc *gc)
{
	...
	cmd->args = realloc_args(cmd->args, count, gc);
	if (!cmd->args)
		return (0);
...
static int	process_word_token(t_cmd *cmd, t_token *current, t_env *env, t_gc *gc)
{
...
static int	process_redirection_token(t_cmd *cmd, t_token **current, t_gc *gc)
{
	if (!handle_redirection(cmd, current, gc))
		return (0);
	return (1);
}
...
t_cmd	*parse_single_command(t_token **tokens, t_env *env, t_gc *gc)
{
	...
	while (current && current->type != TOKEN_PIPE && current->type != TOKEN_EOF)
	{
		...
		{
			if (!process_word_token(cmd, current, env, gc))
				return (free_cmd(cmd), NULL);
		}
		else if (current->type >= TOKEN_REDIRECT_IN 
			&& current->type <= TOKEN_HEREDOC)
		{
			if (!process_redirection_token(cmd, &current, gc))
				return (free_cmd(cmd), NULL);
		}
		else
			return (free_cmd(cmd), NULL);
...
t_cmd	*parse_tokens(t_token *tokens, t_env *env, t_gc *gc)
{
	...
	{
		new_cmd = parse_single_command(&current_token, env, gc);
		if (!new_cmd)
